	...
	int (*comparator)(const void*, const void*))
{
	...
	// насчёт остальных не знаю, но здесь, скорее всего, из-за размера типа char 
	// сдвиг происходит на 1 байт при использовании арифметики указателей
	...
	while (size_prom_arr < elements) {
		...
		while (p_arr < elements) {
			...
			while ((i < p_arr + step) && (j < elements) && (j < p_arr + 2 * step)) {
				...
				// res - результат сравнения, compar работает также как и strcmp
				res = comparator(a, b);
				if (res < 0) {
			...
			//перешагиваем через слитый массив
			p_arr += 2 * step;
		}
